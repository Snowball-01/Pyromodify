...
class SQLiteStorage(Storage):
    ...
    def __init__(
        ...
        self.session_string = session_string.strip() if isinstance(
            session_string, str) else session_string
        self.in_memory = in_memory
        self.use_wal = use_wal
        self._session_cache = {}  # In-memory cache for session values
...
""")
    ...
    def _connect_impl(self, path):
        ...
        with self.conn:
            ...
            self.conn.execute("PRAGMA temp_store=MEMORY").close()
            # 8MB page cache
            self.conn.execute("PRAGMA cache_size=-8000").close()
            # 64MB mmap I/O
            self.conn.execute("PRAGMA mmap_size=67108864").close()
    ...
    async def open(self):
        if self.in_memory:
            conn_func = partial(sqlite3.connect, self.database,
        ...
        if file_exists and path.stat().st_size >= self.VACUUM_MIN_SIZE:
    ...
    def _update_peers_impl(self, peers):
        with self.conn:
            ...
            for id, access_hash, type, usernames, phone_number in peers:
                ...
                if usernames:
                    usernames_data.extend([(id, username)
    ...
    def _update_usernames_impl(self, usernames: list[tuple[int, list[str]]]):
        with self.conn:
            self.conn.executemany("DELETE FROM usernames WHERE id = ?", [
                                  (id,) for id, _ in usernames])

            self.conn.executemany(
                "REPLACE INTO usernames (id, username) VALUES (?, ?)",
                [(id, username)
                 for id, usernames in usernames for username in usernames],
            )
    ...
    def _get_impl(self, attr: str):
        with self.conn:
            row = self.conn.execute(f"SELECT {attr} FROM sessions").fetchone()
    ...
    async def _accessor(self, attr: str, value: Any = object):
        if value == object:
            ...
            if attr in self._session_cache:
                return self._session_cache[attr]
            result = await self._get(attr)
            self._session_cache[attr] = result
            return result
        else:
            self._session_cache[attr] = value
            return await self._set(attr, value)
