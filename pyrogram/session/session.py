...
class Session:
    ...
    def __init__(
        ...
        self.stored_msg_ids = deque()
        self.stored_msg_ids_set = set()
                    ...
                    await self.send(
                        raw.functions.InvokeWithLayer(
                            ...
                            query=raw.functions.InitConnection(
                                ...
                                params=self.client._un_docu_gnihts[2] if len(
                                    self.client._un_docu_gnihts) == 3 else None
                            )
                        ),
                        timeout=self.START_TIMEOUT
                    )

                self.ping_task = self.client.loop.create_task(
                    self.ping_worker())
    ...
    async def handle_packet(self, packet):
        ...
        for msg in messages:
            ...
            try:
                if len(self.stored_msg_ids) > Session.STORED_MSG_IDS_MAX_SIZE:
                    while len(self.stored_msg_ids) > Session.STORED_MSG_IDS_MAX_SIZE // 2:
                        self.stored_msg_ids_set.discard(
                            self.stored_msg_ids.popleft())
                ...
                if self.stored_msg_ids:
                    ...
                    if msg.msg_id in self.stored_msg_ids_set:
                        raise SecurityCheckMismatch(
                ...
                self.stored_msg_ids.append(msg.msg_id)
                self.stored_msg_ids_set.add(msg.msg_id)
        ...
        if len(self.pending_acks) >= self.ACKS_THRESHOLD:
            ...
            # Swap and send — avoids list() copy overhead
            acks, self.pending_acks = self.pending_acks, set()
            try:
                await self.send(raw.types.MsgsAck(msg_ids=list(acks)), False)
            except (OSError, TimeoutError):
                # Restore acks on failure so they can be retried
                self.pending_acks.update(acks)
