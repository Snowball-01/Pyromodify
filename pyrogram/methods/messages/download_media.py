    ...
    async def download_media(
        ...
        if isinstance(message, types.Message):
            if message.new_chat_photo:
                ...
                if story_media and story_media.media:
                    medium = [
                        getattr(story_media, story_media.media.value, None)]
                else:
                    medium = []
                ...
                if any([isinstance(paid_media, (types.PaidMediaPhoto, types.PaidMediaVideo)) for paid_media in message.paid_media.paid_media]):
                    medium = [getattr(paid_media, "photo", (getattr(
                        paid_media, "video", None))) for paid_media in message.paid_media.paid_media]
                elif any([isinstance(paid_media, types.PaidMediaPreview) for paid_media in message.paid_media.paid_media]):
                    medium = [getattr(getattr(paid_media, "minithumbnail"), "data", None)
            ...
            if any([isinstance(paid_media, (types.PaidMediaPhoto, types.PaidMediaVideo)) for paid_media in message.paid_media]):
                medium = [getattr(paid_media, "photo", (getattr(
                    paid_media, "video", None))) for paid_media in message.paid_media]
        ...
        for media in medium:
            if isinstance(media, bytes):
                ...
                os.makedirs(
                    directory, exist_ok=True) if not in_memory else None
