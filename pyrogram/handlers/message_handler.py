...
class MessageHandler(Handler):
    ...
    async def check(self, client: "pyrogram.Client", message: Message):
        ...
        # Cache result on message to avoid double-scanning in resolve_future_or_callback
        message._listener_match_cache = (listener_does_match, listener)
    ...
    async def resolve_future_or_callback(self, client: "pyrogram.Client", message: Message, *args):
        ...
        :return: None
        """
        # Use cached result from check() if available, otherwise re-check
        cached = getattr(message, '_listener_match_cache', None)
        if cached is not None:
            listener_does_match, listener = cached
            message._listener_match_cache = None
        else:
